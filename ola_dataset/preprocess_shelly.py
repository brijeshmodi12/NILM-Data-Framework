# Merges all shelly log files from Ola's Datasets
...
import numpy as np
# --- Config ---
log_folder = r'C:\Users\brind\OneDrive - Universitetet i Oslo\Codes\Alva\datasets\ola\shelly_log_files'
...
output_filepath = r'C:\Users\brind\OneDrive - Universitetet i Oslo\Codes\Alva\datasets\ola\shelly_data.h5'
df_pivot.to_hdf(output_filepath, key="power", mode="w")

print("Data saved to shelly_data.h5")

def infer_sample_rate(index) -> str:
    deltas = pd.Series(index).sort_values().diff().dropna()
    deltas = deltas[deltas > 0]
    return f"{int(deltas.median())}S" if not deltas.empty else "unknown"


infer_sample_rate(df_pivot.index)